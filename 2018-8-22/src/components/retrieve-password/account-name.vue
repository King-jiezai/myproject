...
<template>
    ...
    <div class="account-name">
        ...
        <div class="account-cent">
            ...
            <div class="cnet-name">
                <p class="name">手机号：</p>
                <input type="text" v-model="phone" @blur="blphone">
            ...
            <div class="cnet-code">
                ...
                <div class="obtain-code" :class="{'sens': pro}" @click='codes'>
                    <p v-show='obn'>获取短信验证码</p>
                    <p v-show='sen'>{{this.second}}s</p>
                </div>
                <div class="name-null" v-show="culls">
                    <p>{{this.Verification}}</p>
                </div>
            </div>
        ...
        methods : {
            ...
            async codes () {
                ...
                if (this.nulls == false) {
                    ...
                    if (this.$store.state.Sliding == '') {

                        this.culls = true

                        this.Verification = '请滑动验证码验证'

                    }

                    if (this.$store.state.Sliding == true) {

                        this.culls = false

                        let phoneCode = await this.$api.phoneCode({
                            phone : this.phone
                        })
            ...
            blcode () {
                ...
                if (this.code == '') {
                    ...
                    this.Verification = '请输入短信验证码'

                }

            },

            //如果手机号和验证码不为空且验证码正确就跳转到下一步
            async next () {
                ...
                if (this.nulls == false && this.culls == false) {
                    ...
                    let checkRegister = await this.$api.checkRegister({
                        phone : this.phone,
                        phoneCode : this.code
                    })

                    if (checkRegister.data.status == 0) {

                        this.culls = true

                        this.Verification = '验证码输入错误'

                    }

                    if (checkRegister.data.status == 1) {
