		...
		$.ajax({
			...
			success: function(data) {
				for(var z = 0; z < data.afterSaleOrderVoList.length; z++) {
					...
					if(data.afterSaleOrderVoList[z].status == 3 || data.afterSaleOrderVoList[z].status == 4) {
						...
						agreerefunds.push(list)
						document.getElementById("NodataOne").style.display = "none"

					} else if(data.afterSaleOrderVoList[z].status == 0 || data.afterSaleOrderVoList[z].status == 1 || data.afterSaleOrderVoList[z].status == 2) {
						$(".Nodata").html('暂无数据！')
						console.log(data.afterSaleOrderVoList.length)
					}
				}
				...
				laypage.render({
					elem: 'PageNumberFire',
					count: agreerefunds.length,
					jump: function(obj) {
						//模拟渲染
						document.getElementById('ContentFire').innerHTML = function() {
							var arr = [],
								thisData = agreerefunds.concat().splice(obj.curr * obj.limit - obj.limit, obj.limit);
