					...
					jump: function(obj) {
						...
						document.getElementById('ContentOne').innerHTML = function() {
							...
							layui.each(thisData, function(index, item) {
								...
								/* 状态 */
								if(item.refund == 1) {//仅退款

									if(item.status == 2) {
										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>审核中</div>"

									} else if(item.status == 3) {

										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>审核通过</div>"

									} else if(item.status == 0) {
										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>审核不通过</div>"

									} else if(item.status == 1) {
										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>退款成功</div>"

									}

								} else if(item.refund == 2) {//退货退款
									...
									if(item.status == 2) {
										...
										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>退款关闭</div>"

									}
								} else if(item.refund == 3) {//换货/维修

									if(item.status == 0) {

										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>换货关闭</div>"
										...
										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>已补差价</div>"
...
function Submission(orderNo, productskuId) {
	$.ajax({
		//请求方式
		type: "GET",
		//文件位置
		url: "http://120.78.142.125:8080/dg/afterSaleManager/listAfterSale.do",
		//返回数据格式为json,也可以是其他格式如
		dataType: 'json',
		data: {
		...
		success: function(data) {
			...
			if(data.status == 1) {
				...
				//状态为1，跳转到仅退款审核中页面
				window.open("../../TabPage/OrderManagement/pages/Refund/Refund_Page.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

			} else if(data.status == 2) {
				
				//状态为1，跳转到仅退款审核通过页面
				window.open("../../TabPage/OrderManagement/pages/Refund/Refund_Success.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

			} else if(data.status == 3) {
				
				//状态为0，跳转到仅退款审核不通过页面
				if(data.afterSaleManagerVo.status == 0) {
					
					window.open("../../TabPage/OrderManagement/pages/Refund/Refund_NotSuccess.html?orderNo=" + orderNo + "&productskuId=" + productskuId)
					
				} else if(data.afterSaleManagerVo.status == 1) {
					
					//状态为1，跳转到仅退款成功页面
					window.open("../../TabPage/OrderManagement/pages/Refund/Refund_End.html?orderNo=" + orderNo + "&productskuId=" + productskuId)
				}
			} else if(data.status == 4) {
				
				//状态为4，跳转到退货退款审核中页面
				window.open("../../TabPage/OrderManagement/pages/RefundOfRefunds/Sales_Page.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

			} else if(data.status == 5) {
				
				//状态为5，跳转到退货退款审核通过页面
				window.open("../../TabPage/OrderManagement/pages/RefundOfRefunds/Sales_AddPage.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

			} else if(data.status == 6) {
				
				//状态为5，跳转到退货退款确认收货页面
				window.open("../../TabPage/OrderManagement/pages/RefundOfRefunds/BuyerSReturn.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

			} else if(data.status == 7) {
				
				if(data.afterSaleManagerVo.status == 1) {
					
					//状态为1，跳转到退货退款成功页面
					window.open("../../TabPage/OrderManagement/pages/RefundOfRefunds/CompleteSuccess.html?orderNo=" + orderNo + "&productskuId=" + productskuId)
					
				} else if(data.afterSaleManagerVo.status == 0) {
					
					//状态为0，跳转到退货退款不成功页面
					window.open("../../TabPage/OrderManagement/pages/RefundOfRefunds/CompleteNotSuccess.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

				}
			} else if(data.status == 8) {
				
				//状态为8，跳转到换货/维修审核中页面
				window.open("../../TabPage/OrderManagement/pages/ExchangeGoods/Exchange_Barter.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

			} else if(data.status == 9) {
				
				//状态为9，跳转到换货/维修审核中页面
				window.open("../../TabPage/OrderManagement/pages/ExchangeGoods/Exchange_AddBarter.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

			} else if(data.status == 10) {

				if(data.afterSaleManagerVo.status == 3) {
					
					//状态为3，跳转到换货/维修审核通过页面
					window.open("../../TabPage/OrderManagement/pages/ExchangeGoods/Exchange_Success.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

				} else if(data.afterSaleManagerVo.status == 4) {
					
					//状态为4，跳转到换货/维修补差价页面
					window.open("../../TabPage/OrderManagement/pages/ExchangeGoods/Compensation_Price.html?orderNo=" + orderNo + "&productskuId=" + productskuId)
				}
			} else if(data.status == 11) {
				
				//状态为11，跳转到换货/维修不补差价页面
				window.open("../../TabPage/OrderManagement/pages/ExchangeGoods/Compensation_NotPrice.html?orderNo=" + orderNo + "&productskuId=" + productskuId)
				...
				if(data.afterSaleManagerVo.status == 0) {
					...
					//状态为0，跳转到换货/维修不成功页面
					window.open("../../TabPage/OrderManagement/pages/ExchangeGoods/Exchange_NotSuccess.html?orderNo=" + orderNo + "&productskuId=" + productskuId)

				} else if(data.afterSaleManagerVo.status == 1) {
					
					//状态为0，跳转到换货/维修成功页面
					window.open("../../TabPage/OrderManagement/pages/ExchangeGoods/Exchange_End.html?orderNo=" + orderNo + "&productskuId=" + productskuId)
				}
			...
			$.ajax({
				...
				success: function(data) {
					...
					for(var z = 0; z < data.afterSaleOrderVoList.length; z++) {
						...
						if(data.afterSaleOrderVoList[z].refund == 1) {
					...
					laypage.render({
						elem: 'PageNumberTwo',
						count: onlyrefund.length,
						jump: function(obj) {
							//模拟渲染
							document.getElementById('ContentTwo').innerHTML = function() {
								var arr = [],
									thisData = onlyrefund.concat().splice(obj.curr * obj.limit - obj.limit, obj.limit);
								layui.each(thisData, function(index, item) {
									var rendering = ""
			...
			$.ajax({
				...
				success: function(data) {
					...
					for(var z = 0; z < data.afterSaleOrderVoList.length; z++) {
						...
						if(data.afterSaleOrderVoList[z].refund == 2) {
					...
					//调用分页
					laypage.render({
						elem: 'PageNumberThree',
						count: refunds.length,
						jump: function(obj) {
							//模拟渲染
							document.getElementById('ContentThree').innerHTML = function() {
								var arr = [],
									thisData = refunds.concat().splice(obj.curr * obj.limit - obj.limit, obj.limit);
								layui.each(thisData, function(index, item) {
									var rendering = ""
			...
			$.ajax({
				...
				success: function(data) {
					...
					for(var z = 0; z < data.afterSaleOrderVoList.length; z++) {
						...
						if(data.afterSaleOrderVoList[z].refund == 3) {
					...
					laypage.render({
						...
						jump: function(obj) {
							//模拟渲染
							document.getElementById('ContentFour').innerHTML = function() {
								var arr = [],
									thisData = exchange.concat().splice(obj.curr * obj.limit - obj.limit, obj.limit);
								layui.each(thisData, function(index, item) {
									var rendering = ""
