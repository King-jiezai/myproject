		...
		$.ajax({
			...
			success: function(data) {
				...
				for(var z = 0; z < data.afterSaleOrderVoList.length; z++) {
					...
					if(data.afterSaleOrderVoList[z].status == 0) {
						...
						returnedclose.push(list)
						document.getElementById("NodataFour").style.display = "none"

					} else if(data.afterSaleOrderVoList[z].status == 1 || data.afterSaleOrderVoList[z].status == 3 || data.afterSaleOrderVoList[z].status == 2 || data.afterSaleOrderVoList[z].status == 4) {
						$(".Nodata").html('暂无数据！')
					}
				...
				//调用分页
				laypage.render({
					elem: 'PageNumberEight',
					count: returnedclose.length,
					jump: function(obj) {
						//模拟渲染
						document.getElementById('ContentEight').innerHTML = function() {
							var arr = [],
								thisData = returnedclose.concat().splice(obj.curr * obj.limit - obj.limit, obj.limit);
							layui.each(thisData, function(index, item) {
								...
								if(item.refund == 1) {
								...
								} else if(item.refund == 2) {

									if(item.status == 0) {

										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>退货退款关闭</div>"
									...
									if(item.status == 0) {
										...
										productskuId = item.productskuId
										orderNo = item.orderNo
										Statuss = "<div class='Status' onclick='Submission(" + orderNo + "," + productskuId + ")'>换货/维修关闭</div>"
