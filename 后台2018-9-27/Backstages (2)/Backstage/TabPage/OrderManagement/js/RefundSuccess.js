		...
		$.ajax({
			...
			success: function(data) {
				...
				for(var z = 0; z < data.afterSaleOrderVoList.length; z++) {
					...
					if(data.afterSaleOrderVoList[z].refund == 1 || data.afterSaleOrderVoList[z].refund == 2) {
						...
						if(data.afterSaleOrderVoList[z].status == 1) {
							...
							retuensuccess.push(list)
							document.getElementById("NodataThree").style.display = "none"
				...
				//调用分页
				laypage.render({
					elem: 'PageNumberSeven',
					count: retuensuccess.length,
					jump: function(obj) {
						//模拟渲染
						document.getElementById('ContentSeven').innerHTML = function() {
							var arr = [],
								thisData = retuensuccess.concat().splice(obj.curr * obj.limit - obj.limit, obj.limit);
