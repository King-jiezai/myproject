...
function testtt() {
	$.ajax({
		...
		success: function(data) {
			...
			if(data.afterSaleManagerVo.status == 1 || data.afterSaleManagerVo.status == 4 || data.afterSaleManagerVo.status == 0) {
				$(".closeTime").html(data.refundVo.time)//关闭时间
				$(".Totalrent").html(data.refundVo.money)//退款金额
				$(".rent").html(data.refundVo.totalRent)//补差价租金
				$(".deposit").html(data.refundVo.totalDeposit)//补差价押金
			}

			//退货地址
			if(data.afterSaleManagerVo.status == 2) {
				$(".reAdRgName").html(data.logisticsVo.name)//收货人
				$(".reAdRgAddress").html(data.logisticsVo.province)//省市区
				$(".DetailedAddress").html(data.logisticsVo.address)//详细地址
				$(".reAdRgPhones").html(data.logisticsVo.cellPhone)//联系电话
				$(".reAdRgCodes").html(data.logisticsVo.postCode)//邮政编号
			}

			if(data.afterSaleManagerVo.status == 3) {
				$(".Through_Time").html(data.refundVo.time)//通过时间
			}
...
function Review() {
	...
	var totalRent = $("#coRent").val()//补差价租金

	var totalDeposit = $("#coDeposit").val()//补差价押金
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(data.status == 1) {
				...
				//		setTimeout(Submission(orderNo, productskuId) , 5000);
...
function disagree() {
	...
	var tests = document.getElementById("coMoney").style.display
	if(test === "none" || tests === "none") {
		document.getElementById("diOp").style.display = "block"
		document.getElementById("coMoney").style.display = "none"
...
function cancel() {
	...
	document.getElementById("coMoney").style.display = "block"
...
/* 确认收货 */
function coSubmission() {
	$.ajax({
		//请求方式
		type: "POST",
		//文件位置
		url: "http://120.78.142.125:8080/dg/afterSaleManager/confirmReceipt.do",
		//url: "http://192.168.1.172:8080/dg/afterSaleManager/confirmReceipt.do",
		//返回数据格式为json,也可以是其他格式如
		dataType: 'json',
		xhrFields: {
			withCredentials: true
	...
	layer.open({
		...
		yes: function() {
			...
			$.ajax({
				...
				type: "POST",
				//文件位置
				url: "http://120.78.142.125:8080/dg/afterSaleManager/updateAddress.do",
				//传值给后台
				data: {
	...
	layer.open({
		...
		yes: function() {
			...
			$.ajax({
				...
				type: "POST",
				//文件位置
				url: "http://120.78.142.125:8080/dg/afterSaleManager/addAddress.do",
				//传值给后台
				data: {
					receiverName: receiverName,//联系人
					receiverMobile: receiverMobile,//联系电话
