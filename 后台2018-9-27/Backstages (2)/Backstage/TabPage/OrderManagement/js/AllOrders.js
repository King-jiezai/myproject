  ...
  /* 打印数据 */
	function PrintingData(data){
    	...
    	OrOp = "<div class='orOp'>"+ OrDe +"</div>"
			...
			/* 优惠 */
			Discount = "<div class='Discount'>优惠数据："+data.data.list[i].postage +"</div>"
			/* 运费 */
...
function OrderQuery(){
    ...
    success: function(data){
    	...
    	if( data.data.status == 10 ){
    		OrDeHa= "<div class='OrDeHa'><span class='curpoi'>订单关闭</span></div>"
    		BiEx = "<div class='BiEx'><span class='curpoi' onclick='expediting("+ data.data.orderNo +")'>订单催付</span></div>"
    		OrderDetails = "<div class='OrDe'><span class='curpoi'>订单详情<span></div>"
    		OrDe = "<div>"+ OrderDetails + BiEx + OrDeHa +"</div>"
    	...
    	OrOp = "<div class='orOp'>"+ OrDe +"</div>"
			...
			/* 分期 */
			var StagingState
			if( data.data.orderItemVoList.length == 1 ){
				if( data.data.orderItemVoList[0].staging != 1 ){
					StagingState = "<div class='StagingState'>已付/总额：￥"+ data.data.orderItemVoList[0].paidMoney +" / "+ data.data.orderItemVoList[0].totalMoney +"</div>"
				}
			} else {
				StagingState = 123123
			}
			...
			/* 改价按钮 */
			if( data.data.status == 10 ){
    		rePr = "<div class='rePr'><span onclick='PriceChange("+ data.data.orderNo +")'>改价</span></div>"
    	} else {
    		rePr = ""
    	}
			/* 判断付款方式  1-支付宝 2-微信 3-银联 */
    	if( data.data.paymentType == 1 ){
    		iconfont = "<i class='iconfont icon-zhifubao'></i>"
    	} else if( data.data.list.paymentType == 2 ){
    		iconfont = "<i class='iconfont icon-wechat'></i>"
    	} else if( data.data.list.paymentType == 3 ){
    		iconfont = "<i class='iconfont icon-buoumaotubiao26'></i>"
    	}
			money = "<div class='money'>"+ iconfont + data.data.payment +"</div>"
			NotrMo = "<div class='No_trMo'>"+ money + rePr + Expresscost + Discount + StagingState +"</div>"
			
			/* 订单内容 */
			coInNumber = ''
			for( var a = 0; a < data.data.orderItemVoList.length; a++  ){
				/* 单价 * 数量 */
				sePr = "<div class='sePr'>3333.49*1(死数据)</div>"
				...
				coFu = "<div class='coFu'>"+
							 	"<span>押金（单件）：XXXXXX</span>"+
								"<span>租金（单件）：XXXXXXX</span>"+
								"<span>租期天数："+ data.data.orderItemVoList.number +"</span>"+
							 "</div>"
				...
				coInlf = "<div class='coInlf'>"+ shNa + coMo + coFu +"</div>"
				shImg = "<div class='shImg'><img src='"+ data.data.orderItemVoList[0].productImage +"'/></div>"  //商品图片
				coIn = "<div class='coIn'>"+ shImg + coInlf + StartEnd + sePr +"</div>"
				coInNumber = coInNumber + coIn
			}
			commodityNumber ="<div class='commodity_Number'>"+ coInNumber +"</div>"
			...
			/* 订单详情 */
			SeSNo = "<div class='SeSNo'>卖家备注：<span></span>"+
								"<i class='layui-icon curpoi' onclick='moSellerMessage("+ data.data.orderNo +")'>&#xe642;</i></span></div>"
			        +"</div>"
			buSNo = "<div class='buSNo'>买家备注：<span>"+ data.data.message +"</span></div>"
			reIn = "<div class='reIn'> 收货信息：<span id='reIn"+ data.data.orderNo +"'>"+ data.data.shippingVo.receiverName+ " " +  //收件人姓名
						 	data.data.shippingVo.receiverMobile + " " + data.data.shippingVo.receiverPhone + " " +  //收件人固定电话  移动电话
						 	data.data.shippingVo.receiverRegion + " " + data.data.shippingVo.receiverArea + " " +  //地区  省份-市-区
						 	data.data.shippingVo.receiverAddress + " " + data.data.shippingVo.receiverZip  //详细地址  邮政编码
						 +"</span>"+
						 	"<textarea id='input'></textarea>"+
						 	"<span class='reInOp curpoi' onclick='copy("+ data.data.orderNo +")'>复制</span>|<span class='reInOp curpoi' onclick='DetailsModify("+ data.data.orderNo +")'>修改</span>"
						 +"</div>"
			middle = "<div class='middle'>"+ reIn + buSNo + SeSNo +"</div>"
    ...
    success: function(data){
    	...
    	/* 弹窗内容 */
		    ...
		    yes: function(){
		      layer.closeAll();
				  var body = layer.getChildFrame('body');
					var province = body.find('.zcityItem:nth-child(1) input').val()  //省
				  var city = body.find('.zcityItem:nth-child(2) input').val()  //市
				  var Area = body.find('.zcityItem:nth-child(3) input').val()  //区
...
/* 订单详情 */
function OrderDetailsWindow(orderNo){
	layer.open({
    type: 2,
    area: ['500px','340px'],
    scrollbar: false,
    ...
    success: function(data){
    	obtain = data
		    ...
		    yes: function(){
		    	layer.closeAll();
		      var body = layer.getChildFrame('body');
		      var expressName  //快递名称
		      var expressSelect  //快递名称代码
		      var expressNumber  //快递编号
		      expressSelect = body.find('#expressSelect option:selected').val()
		      expressNumber = body.find('.expressNumber input').val()
		      
		      if( expressSelect == "debangwuliu" ){
		      	expressName = "德邦"
		      } else if( expressSelect == "ems" ){
		      	expressName = "邮政"
		      } else if( expressSelect == "guotongkuaidi" ){
		      	expressName = "国通"
		      } else if( expressSelect == "huitongkuaidi" ){
		      	expressName = "汇通"
		      } else if( expressSelect == "quanfengkuaidi" ){
		      	expressName = "全峰"
		      } else if( expressSelect == "shunfeng" ){
		      	expressName = "顺丰"
		      } else if( expressSelect == "shentong" ){
		      	expressName = "申通"
		      } else if( expressSelect == "tiantian" ){
		      	expressName = "天天"
		      } else if( expressSelect == "yunda" ){
		      	expressName = "韵达"
		      } else if( expressSelect == "yuantong" ){
		      	expressName = "圆通"
		      } else if( expressSelect == "youzhengguonei" ){
		      	expressName = "邮政包裹/平邮"
		      } else if( expressSelect == "youshuwuliu" ){
		      	expressName = "优速"
		      } else if( expressSelect == "zhongtong" ){
		      	expressName = "中通"
		      } else if( expressSelect == "zhaijisong" ){
		      	expressName = "宅急送"
		      } else if( expressSelect == "zhongtiewuliu" ){
		      	expressName = "中铁物流"
		      } else if( expressSelect == "ztky" ){
		      	expressName = "中铁快运"
		      }
		      /* 发货执行函数 */
					$.ajax({
						...
						type:"POST",
						//文件位置
						url:"http://120.78.142.125:8080/dg/manage/order/sendGoods.do",
						data: {
							orderNo: orderNo, //订单号
							logisticsCompany: expressName,  //快递公司名称
							logisticsCoding: expressSelect,  //快递公司名称编号
							logisticsNumber: expressNumber  //运单号
						},
						xhrFields: {
